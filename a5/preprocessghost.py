...
def process(file):
    ...
    for data in json_data:
        ...
        temp["tsval"] = data['_source']['layers']["tcp"]["tcp.options_tree"]["tcp.options.timestamp_tree"]["tcp.options.timestamp.tsval"]
        try:
    ...
    for i in range(len(streamList)):
        ...
        tcp["Website Connection"] = streams
        website_list.append(tcp)
    with open('ghost.json', 'w') as f:
        json.dump(website_list, f, indent=2)
    return None


process("2024Json.json")
